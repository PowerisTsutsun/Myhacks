...
export function FaqManager({ initialData }: { initialData: Row[] }) {
  ...
  return (
    <div>
      ...
      {error && (
        <div className="mb-4 rounded-xl border px-3 py-3 text-sm text-red-100" role="alert" style={{ background: "rgba(251,113,133,0.14)", borderColor: "rgba(251,113,133,0.35)" }}>
          {error}
        </div>
      )}
      ...
      {!showForm ? (
        <>
          ...
          {rows.length === 0 ? (
            <div className="admin-surface rounded-2xl border p-10 text-center" style={{ borderColor: "rgba(52,211,153,0.18)" }}>
              <p className="mb-3 text-semantic-text-muted">No FAQ items yet.</p>
              <Button onClick={openNew} variant="outline">Add your first FAQ</Button>
            </div>
          ) : (
            <div className="admin-surface divide-y rounded-2xl border" style={{ background: "rgba(12,25,41,0.95)", borderColor: "rgba(52,211,153,0.18)", borderTopColor: "rgba(52,211,153,0.18)" }}>
              {rows.map((row) => (
                <div key={row.id} className="flex items-start justify-between gap-4 p-4">
                  <div className="flex-1 min-w-0">
                    <div className="flex items-center gap-2 flex-wrap mb-1">
                      <span className="font-medium text-white text-sm">{row.question}</span>
                    ...
                    <p className="text-semantic-text-muted text-xs line-clamp-2">{row.answer}</p>
                  </div>
        ...
        <div className="admin-surface max-w-2xl rounded-2xl border p-5" style={{ borderColor: "rgba(52,211,153,0.18)" }}>
          <div className="flex items-center justify-between mb-4">
            <h2 className="font-semibold text-white">{editing ? "Edit FAQ" : "New FAQ Item"}</h2>
            <Button size="sm" variant="ghost" onClick={() => setShowForm(false)}>Cancel</Button>
          </div>
          <form onSubmit={handleSubmit(onSubmit)} className="space-y-4">
            <Textarea dark label="Question" required error={errors.question?.message} {...register("question")} />
            <Textarea dark label="Answer" required className="min-h-[120px]" error={errors.answer?.message} {...register("answer")} />
            <Input dark label="Category" hint='e.g. "general", "teams", "prizes"' error={errors.category?.message} {...register("category")} />
